...
# Compares Full, Random, Proposed, Hybrid (Proposed→Random)
# ------------------------------------------------------------
import argparse
from pathlib import Path
import numpy as np
import matplotlib.pyplot as plt
from dfl_har_hybrid import run_experiment
...
def parse_args():
    ...
    p.add_argument("--tau", type=int, default=70, help="percentile τ (threshold mode)")
    p.add_argument("--rho", type=float, default=0.3, help="top-ρ selection ratio")
    p.add_argument("--rounds", type=int, default=300)
    # hybrid options
    p.add_argument("--hybrid_mode", type=str, default="round", choices=["round","plateau"])
    p.add_argument("--t_switch", type=int, default=220)
    p.add_argument("--plateau_win", type=int, default=20)
    p.add_argument("--plateau_eps", type=float, default=1e-3)
    return p.parse_args()
...
def plot_histories(hists, labels, title_suffix=""):
    ...
    for h, lb in zip(hists, labels):
        ...
        if "switch_round" in h and h["switch_round"] >= 0:
            sr = h["switch_round"]
            plt.axvline(sr, linestyle="--", alpha=0.5)
...
if __name__ == "__main__":
    ...
    if not xtrain.exists() or not xtest.exists():
        raise FileNotFoundError(
            f"[DATA_ROOT 점검] '{DATA_ROOT}' 아래에 'train/X_train.txt' 또는 'test/X_test.txt'가 없습니다.\n"
            f"현재 확인된 경로:\n  - {xtrain}\n  - {xtest}"
        )
    ...
    common = dict(
        ...
        rho=args.rho, selection_mode=args.selection_mode, tau_percentile=args.tau,
    ...
    # Hybrid: proposed → random
    hist_hybr = run_experiment(
        **common,
        policy="hybrid",
    ...
    plot_histories(
        [hist_full, hist_rand, hist_prop, hist_hybr],
        labels=["Full", "Random", "Proposed", "Hybrid(P→R)"],
